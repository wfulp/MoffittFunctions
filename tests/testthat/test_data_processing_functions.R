...
test_that(".rm_na_and_check returning p value", {
  ...
  #x only one value
  expect_identical(.rm_na_and_check(rep(1,6), rep(1:2,3)), expected = 1)
  ...
  expect_identical(.rm_na_and_check(rep(1:2,3),rep(1,6)), expected = NA)
  ...
  expect_identical(.rm_na_and_check(rep(1:2,3),rep(1,6), y_type = 'cont'), expected = 1)
