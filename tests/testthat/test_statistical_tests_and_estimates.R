...
test_that("round_away_0 testing various options (no errors)", {
  ...
  expect_identical(object = round_away_0(x),
               expected = c(0,2,3,3,4,4,NA))

  #Note 4.05 goes to 4.1 as expected
  expect_identical(object = round_away_0(x, digits = 1),
               expected = c(0,2.5,2.5,2.5,3.5,4.1,NA))
  #Testing for trailing zeros
  expect_identical(object = round_away_0(x, digits = 2, trailing_zeros = TRUE),
...
test_that("round_away_0 testing various options (no errors)", {
  ...
  #Note 2.5 goes to 3 as expected
  expect_identical(object = .round_if_numeric(x),
               expected = c(0,2,3,3,4,4,NA))
  #Note 4.05 goes to 4.1 as expected
  expect_identical(object = .round_if_numeric(x, digits = 1),
               expected = c(0,2.5,2.5,2.5,3.5,4.1,NA))
  #Testing for character
  expect_identical(object = .round_if_numeric(c(NA,letters[1:5])),
               expected = c(NA,letters[1:5]))
  #Testing for trailing zeros
  expect_identical(object = .round_if_numeric(x, digits = 2, trailing_zeros = TRUE),
               expected = c("0.00", "2.50", "2.50", "2.50", "3.50", "4.05", NA))
...
test_that("two_samp_cont_test testing various options (no errors)", {
  ...
  expect_identical(object = two_samp_cont_test(x = x[-(11:12)], y = y[-(11:12)], method = 't', paired = TRUE, verbose = T),
  ...
  expect_identical(object = two_samp_cont_test(x = x, y = y, method = 't', paired = FALSE, verbose = T, var.equal = T),
               expected = t.test(x~factor(y), paired = F, var.equal = T)$p.value)
  #Testing alternative param can be used
  expect_identical(object = two_samp_cont_test(x = x, y = y, method = 't', paired = FALSE, verbose = T, alternative = 'less'),
               expected = t.test(x~factor(y), paired = F, alternative = 'less')$p.value)

  #Testing t.test where both levels of y have a single x value
  expect_identical(object = two_samp_cont_test(x = rep(1:2,5), y = rep(0:1,5), method = 't'), expected = NA)
...
test_that("two_samp_cont_test throwing internal .rm_na_and_check checking errors", {
  ...
  expect_identical(object = two_samp_cont_test(x = c(rep(1,20),rep(NA,20)), y = c(rep(NA,20),rep(1,20))), expected = NA)
  ...
  expect_identical(object = two_samp_cont_test(x = rep(1,22), y = y), expected = 1)
  ...
  expect_identical(object = two_samp_cont_test(x = x, y = rep(1,22)), expected = NA)
...
test_that("two_samp_bin_test testing various options (no errors)", {
  ...
  # Barnard
  expect_identical(object = two_samp_bin_test(x = x, y = y, method = 'barnard', alternative = 'two.sided'),
               expected = Exact::exact.test(table(data.frame(x,y)), method = 'Z-pooled', to.plot = FALSE, alternative = 'two.sided')$p.value)
  # Fisher
  expect_identical(object = two_samp_bin_test(x = x, y = y, method = 'fisher', alternative = 'two.sided'),
               expected = fisher.test(x, y, alternative = 'two.sided')$p.value)
  # Chi-sq
  expect_identical(object = two_samp_bin_test(x = x, y = y, method = 'chi.sq', alternative = 'two.sided'),
               expected = chisq.test(x, y)$p.value)
  # McNemar(paired data)
  expect_identical(object = two_samp_bin_test(x = x[!is.na(y)], y = y[!is.na(y)], method = 'mcnemar', alternative = 'two.sided'),
               expected = mcnemar.test(paired_data_here$a, paired_data_here$b)$p.value)

  # Directional, along with factor values
  expect_identical(object = two_samp_bin_test(x = x, y = y, method = 'barnard', alternative = 'less'),
               expected = Exact::exact.test(table(data.frame(x,y)), method = 'Z-pooled', to.plot = FALSE, alternative = 'less')$p.value)
  expect_identical(object = two_samp_bin_test(x = factor(x), y = y, method = 'barnard', alternative = 'less'),
               expected = Exact::exact.test(table(data.frame(x,y)), method = 'Z-pooled', to.plot = FALSE, alternative = 'less')$p.value)
  expect_identical(object = two_samp_bin_test(x = x, y = y, method = 'barnard', alternative = 'greater'),
               expected = Exact::exact.test(table(data.frame(x,y)), method = 'Z-pooled', to.plot = FALSE, alternative = 'greater')$p.value)
  expect_identical(object = two_samp_bin_test(x = factor(x, levels = 1:0), y = y, method = 'barnard', alternative = 'less'),
               expected = Exact::exact.test(table(data.frame(x,y)), method = 'Z-pooled', to.plot = FALSE, alternative = 'greater')$p.value)
})
...
test_that("two_samp_bin_test throwing internal .rm_na_and_check checking errors", {
  ...
  #Testing case where no non-missing pairs
  expect_identical(object = two_samp_bin_test(x = c(rep(1,20),rep(NA,20)), y = c(rep(NA,20),rep(1,20)), method = 'barnard'), expected = NA)
  expect_message(object = two_samp_bin_test(x = c(rep(1,20),rep(NA,20)), y = c(rep(NA,20),rep(1,20)), method = 'barnard', verbose = T),
                 regexp = 'There are no observations with non-missing values of both "x" and "y", so p=NA returned')

  #Testing case where all x have same value
  expect_identical(object = two_samp_bin_test(x = rep(1,22), y = y, method = 'barnard'), expected = 1)
  expect_message(object = two_samp_bin_test(x = rep(1,22), y = y, method = 'barnard', verbose = T),
                 regexp = '"x" only has 1 distinct value when considering non-missing values of "y", so p=1 returned')

  #Testing case where all y have same value
  expect_identical(object = two_samp_bin_test(x = x, y = rep(1,22), method = 'barnard'), expected = NA)
